    ...
    return fs.promises
      ...
      .then((files) => {
        ...
        return Promise.all(children).then((children) => {
          ...
          const hash = new HashedFolder(
            name,
            children.filter(notUndefined),
            options,
            isRootElement,
          )
          return hash
  ...
  async function symLinkResolve(name, dir, target, options, isRootElement) {
    ...
    try {
      ...
      if (options.symbolicLinks.ignoreTargetContentAfterError) {
        log.symlink(
          `Ignoring error "${err.code}" when hashing symbolic link ${name}`,
          err,
        )
        const hash = crypto.createHash(options.algo)
        if (
        ...
        log.symlink(
          `Error "${err.code}": When hashing symbolic link ${name}`,
          err,
        )
        throw err
      }
...
function parseParameters(args) {
  ...
  const options = {
    ...
    symbolicLinks: Object.assign(
      {},
      defaultOptions.symbolicLinks,
      options_.symbolicLinks,
    ),
  }
...
function reduceGlobPatterns(globs) {
  if (isFunction(globs)) {
    ...
    // console.log(globs, 'Params', regex)
    return (param) => {
